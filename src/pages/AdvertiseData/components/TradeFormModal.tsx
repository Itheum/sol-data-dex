...
export const TradeFormModal: React.FC<TradeFormProps> = (props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      onChangeDataNFTMarshalService(getApiDataMarshal());
  ...
  const onChangeDataNFTMarshalService = (value: string) => {
    ...
    checkUrlReturns200(`${getApiDataMarshal()}/health-check`).then(({ isSuccess }) => {
  ...
  const onChangeDataNFTImageGenService = () => {
    ...
    checkUrlReturns200(`${getApiDataDex()}/health-check`).then(({ isSuccess }) => {
