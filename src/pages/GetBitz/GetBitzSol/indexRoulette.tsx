  ...
  const OverLay = ({ label = "", ActionComponent = null }: { label?: string; ActionComponent?: ReactNode }) => {
    return (
      <div className="relative">
        <div className={cn("absolute z-5 w-full h-full rounded-[3rem]", colorMode === "dark" ? "bg-black/90" : "bg-white/90", modalMode ? "rounded" : "")}>
          <div className="flex w-full h-full items-center justify-center">
            <div className="text-3xl md:text-5xl flex flex-col items-center justify-center text-white ">
              <p className={`${colorMode === "dark" ? "text-white" : "text-black"} my-4 text-xl md:text-3xl`}>{label} </p> {ActionComponent}
            </div>
          </div>
        </div>
        <div className={`${colorMode === "dark" ? "bg-black/90" : "bg-white/90"} min-h-[350px] rounded-[3rem] w-full`}></div>
      </div>
    );
  };
      ...
      return (
        <>
          <div className="relative overflow-hidden flex flex-col justify-center items-center bgx-red-700">
            <div className="text-xl text-center relative top-[30px] font-bold">Spin for Your Chance To Win BiTz XP!</div>
            <div className="gameCanvas bgx-green-500 flex flex-col md:flex-row">
              ...
              <div className="flex flex-col justify-center items-center pb-[10px] md:pb-[0px]">
                <Button
                  onClick={playGame}
                  isDisabled={isFetchingDataMarshal || spinComplete || cooldown > 0 || gameInProgress}
                  m="auto"
                  colorScheme="teal"
                  variant="outline"
                  ...
                  {isFetchingDataMarshal ? (
                    <span className="flex items-center justify-center">
                      <svg
                        className={`animate-spin -ml-1 mr-2 h-4 w-4 ${colorMode === "dark" ? "text-white" : "text-black"}`}
                    ...
                    {viewDataRes.data.gamePlayResult.triedTooSoonTryAgainInMs === -1 && (
                      <div className="flex flex-col justify-around items-center text-center">
                        ...
                        <div
                          ...
                          className="cursor-pointer border border-primary rounded-md p-2 mt-2 text-black bg-teal-200 hover:bg-teal-300">
                          Close & Return
                        </div>
                      </div>
                    )}
                    ...
                    if (props.completed) {
                      ...
                      return (
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <div
                            onClick={onHideBitzModel}
                            className="cursor-pointer border border-primary rounded-md p-2 text-black bg-teal-200 hover:bg-teal-300">
                            Close & Return
                          </div>
                        </div>
                      );
